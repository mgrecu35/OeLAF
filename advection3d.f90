...
real function interp(field,nz,ny,nx,ix,iy,iz)
  ...
  sy1=1-sy
  sz1=1-sz
  s0=field(iz0,iy0,ix0)*sx1*sy1+field(iz0,iy0+1,ix0)*(1-sx1)*sy1+&
       field(iz0,iy0,ix0+1)*sx1*(1-sy1)+&
       field(iz0,iy0+1,ix0+1)*(1-sx1)*(1-sy1)
  s1=field(iz0+1,iy0,ix0)*sx1*sy1+field(iz0+1,iy0+1,ix0)*(1-sx1)*sy1+&
       field(iz0+1,iy0,ix0+1)*sx1*(1-sy1)+&
...
real function interp2(field,ny,nx,ix,iy)
  ...
  sx1=1-sx
  sy1=1-sy
  s0=field(iy0,ix0)*sx1*sy1+field(iy0+1,ix0)*(1-sx1)*sy1+&
       field(iy0,ix0+1)*sx1*(1-sy1)+&
...
subroutine trace3dball(u,v,w,dz,zm,mapf,&
     ...
     npart, posPartOut, lonOut, latOut, hOut)
  implicit none
  ...
  do ip=1,npart
     ...
     hOut(1,ip)=interp(zt,nz,ny,nx,ix(ip),iy(ip),iz(ip))
  enddo
